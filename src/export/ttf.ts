...
function makeOs2Table(
  ...
  result.u16(options.fsSelection | 0x0080);
  ...
  const winAscent = Math.min(Math.max(options.ascender, bounds.yMax, 0), Math.round(options.unitsPerEm * 0.8));
  const winDescent = Math.min(Math.max(-options.descender, -bounds.yMin, 0), Math.round(options.unitsPerEm * 0.2));
  result.u16(toUint16(winAscent, "usWinAscent"));
  result.u16(toUint16(winDescent, "usWinDescent"));
  result.u32(1);
  result.u32(0);
  result.i16(0);
